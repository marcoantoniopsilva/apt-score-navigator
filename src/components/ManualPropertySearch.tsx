...
const parseRegion = (region: string) => {
  const removeAccents = (text: string) => {
    return text.replace(/[áàâãä]/g, 'a').replace(/[éèêë]/g, 'e').replace(/[íìîï]/g, 'i').replace(/[óòôõö]/g, 'o').replace(/[úùûü]/g, 'u').replace(/[ç]/g, 'c').replace(/[ñ]/g, 'n').replace(/[ÁÀÂÃÄ]/g, 'A').replace(/[ÉÈÊË]/g, 'E').replace(/[ÍÌÎÏ]/g, 'I').replace(/[ÓÒÔÕÖ]/g, 'O').replace(/[ÚÙÛÜ]/g, 'U').replace(/[Ç]/g, 'C').replace(/[Ñ]/g, 'N');
  };
  const normalize = (text: string) => removeAccents(text).toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9\-]/g, '');
...
export const ManualPropertySearch = ({
  onAddProperty
}: ManualPropertySearchProps) => {
    ...
    return <Alert className="mb-6">
        ...
        <AlertDescription>
          <div className="space-y-2">
            ...
            {userProfile.faixa_preco && <p><strong>Faixa de preço:</strong> {userProfile.faixa_preco}</p>}
            {topCriteria.length > 0 && <p><strong>Critérios importantes:</strong> {topCriteria.map(c => c.criterio_nome).join(', ')}</p>}
          </div>
        </AlertDescription>
      </Alert>;
  };
  return <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {externalPortals.map(portal => <Card key={portal.name} className="hover:shadow-md transition-shadow">
            ...
            <CardContent className="pt-0">
              <Button variant="outline" className="w-full" onClick={() => window.open(buildSearchUrl(portal), '_blank')}>
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">Clique aqui para adicionar uma nova propriedade (copie o link do anúncio e cole a seguir)</p>
        </CardHeader>
        <CardContent className="space-y-4">
          <Button onClick={handleExtractProperty} className="w-full">
